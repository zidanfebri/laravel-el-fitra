...
<head>
    ...
    <title>Edit Testimoni - El-Fitra</title>
...
<body>
    ...
    <div class="content">
        ...
        <div class="form-container">
            <form action="{{ route('admin.update-testimoni', $testimoni->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="nama" class="form-label">Nama</label>
                    <input type="text" class="form-control" id="nama" name="nama" value="{{ old('nama', $testimoni->nama) }}" required>
                </div>
                <div class="mb-3">
                    <label for="keterangan" class="form-label">Keterangan</label>
                    <textarea class="form-control" id="keterangan" name="keterangan" rows="5" required>{{ old('keterangan', $testimoni->keterangan) }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="tanggal" class="form-label">Tanggal</label>
                    <input type="date" class="form-control" id="tanggal" name="tanggal" value="{{ old('tanggal', $testimoni->tanggal) }}" required>
                </div>
                <div class="mb-3">
