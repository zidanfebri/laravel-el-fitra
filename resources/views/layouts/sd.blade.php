        ...
        <div class="toast-container position-fixed top-0 end-0 p-3">
            <div class="toast animate__animated animate__fadeInUp" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="true" data-bs-delay="5000">
                <div class="toast-header">
                    <img src="{{ asset('images/elfitra.jpeg') }}" alt="{{ __('messages.site_name_menu') }}" width="20" class="me-2 rounded-circle">
                ...
                <ul class="navbar-nav ms-auto align-items-center gap-3">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown">
                            {{ __('messages.profil_sd') }} <i class="bi bi-chevron-down ms-1"></i>
                        </a>
                        <ul class="dropdown-menu border-0 shadow">
                            <li><a class="dropdown-item" href="{{ route('jenjang.sd') }}#tentang-kami">{{ __('messages.about_us') }}</a></li>
                            <li><a class="dropdown-item" href="{{ route('jenjang.sd') }}#vision-mission">{{ __('messages.vision_mission') }}</a></li>
                            <li><a class="dropdown-item" href="{{ route('jenjang.sd') }}#fasilitas">{{ __('messages.facilities') }}</a></li>
                        </ul>
                    </li>
                    ...
                    <li class="nav-item">
                        <a class="nav-link btn-orange-nav" href="{{ route('pendaftaran.check') }}">{{ __('messages.registration') }}</a>
                    </li>
                    <li class="nav-item">
                        @auth
                            <form action="{{ route('logout') }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="nav-link btn-orange-nav btn-logout-dark">{{ __('messages.logout') }}</button>
                            </form>
                        @else
                            <a class="nav-link btn-orange-nav" href="{{ route('login') }}">{{ __('messages.login') }}</a>
                        @endauth
                    </li>
                </ul>
            </div>
    ...
    <footer class="footer py-4">
        <div class="container-fluid px-lg-5 px-3"> 
            ...
            <div class="footer-bottom text-center">
                ...
                <p class="mb-0 mt-2">
                    {{ __('messages.total_visits') }}: <span class="visit-count">{{ \App\Models\Visit::first()->sd_count ?? 0 }}</span>
                </p>
            </div>
        </div>
    </footer>
